...
public final class GroovyCastingTransformer implements ASTTransformer {
    ...
    public Expression visit(CastingExpression expression) {
        if (expression.getCastingType().equals(CastingExpression.CastingType.SAFE_AS)) {
            ...
            return new TernaryExpression(new InstanceOfExpression(expression.getExpression(), expression.getType()), new CastingExpression(CastingExpression.CastingType.AS, expression.getType(), expression.getExpression()), new NullLiteralExpression()).accept(this);
        } else {
